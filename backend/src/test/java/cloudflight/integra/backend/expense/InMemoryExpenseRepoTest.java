 ...
 *
 * <p>Tests cover CRUD operations: - addExpense - updateExpense - findById - findAllByUserId -
 * deleteById
 *
 * <p>Also verifies that exceptions are thrown for invalid operations.
 */
public class InMemoryExpenseRepoTest {
  ...
  void shouldReturnAllExpensesForUser() {
    repository.addExpense(
        new ExpenseDTO(null, 1L, new BigDecimal("50"), "Transport", LocalDate.now(), null));
    repository.addExpense(
        new ExpenseDTO(null, 2L, new BigDecimal("30"), "Food", LocalDate.now(), null));
    repository.addExpense(
        new ExpenseDTO(null, 1L, new BigDecimal("20"), "Coffee", LocalDate.now(), null));
  ...
  void shouldUpdateExistingExpense() {
    ExpenseDTO expense =
        repository.addExpense(
            new ExpenseDTO(null, 1L, new BigDecimal("25"), "Snack", LocalDate.now(), null));
  ...
  void updateNonExistingExpenseShouldThrow() {
    ExpenseDTO expense =
        new ExpenseDTO(999L, 1L, new BigDecimal("30"), "Snack", LocalDate.now(), null);
  ...
  void shouldDeleteExistingExpense() {
    ExpenseDTO expense =
        repository.addExpense(
            new ExpenseDTO(null, 1L, new BigDecimal("15"), "Snack", LocalDate.now(), null));
