 ...
 *
 * <p>Tests CRUD operations via MockMvc using ExpenseDto objects.
 */
...
//    private MockMvc mockMvc;
  //  private ObjectMapper objectMapper;
    ...
    /**
     * Sets up the in-memory repository, validator, service, controller, and MockMvc before each
     * test. Configures Spring Validator to properly handle @Valid annotations in controller.
     */
    // @BeforeEach
    // void setUp() {
      //  InMemoryExpenseRepositoryImpl repository = new InMemoryExpenseRepositoryImpl();

        // repository.addExpense(new ExpenseDTO(1L, 1L, new BigDecimal("50.00"), "Food",
        // LocalDate.now(), "Lunch"));
        // repository.addExpense(new ExpenseDTO(2L, 1L, new BigDecimal("150.00"), "Transport",
        // LocalDate.now(), "Taxi"));
        // repository.addExpense(new ExpenseDTO(3L, 2L, new BigDecimal("200.00"), "Shopping",
        // LocalDate.now(), "Clothes"));

        // LocalValidatorFactoryBean validator = new LocalValidatorFactoryBean();
        // validator.afterPropertiesSet();

        // ExpenseService expenseService = new ExpenseServiceImpl(repository, new
        // ExpenseValidator());
...
//      .andDo(print())
  //              .andExpect(status().isOk())
    ...
    // void testCreateExpense() throws Exception {
      ...
      //  ExpenseDTO newExpense = new ExpenseDTO(null, 1L, new BigDecimal("75.00"), "Groceries",
      // LocalDate.now(), "Weekly shopping");
    ...
    // void testUpdateExpense() throws Exception {
      //  ExpenseDTO updatedExpense = new ExpenseDTO(1L, 1L, new BigDecimal("55.00"), "Food",
      // LocalDate.now(), "Lunch updated");
