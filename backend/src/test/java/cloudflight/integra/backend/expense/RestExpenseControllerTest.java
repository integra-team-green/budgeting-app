...
class RestExpenseControllerTest {
  ...
  void getExpenseById_Success() throws Exception {
    mockMvc
        .perform(get("/api/v1/expenses/{id}", testExpense.getId()))
  ...
  void updateExpense_Success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/expenses/{id}", testExpense.getId())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void deleteExpense_Success() throws Exception {
    mockMvc
        .perform(delete("/api/v1/expenses/{id}", testExpense.getId()))
