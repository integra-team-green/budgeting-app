...
class RestExpenseControllerTest {
  ...
  void setUp() {
    ...
    repository.addExpense(
        new ExpenseDTO(1L, 1L, new BigDecimal("50.00"), "Food", LocalDate.now(), "Lunch"));
    repository.addExpense(
        new ExpenseDTO(2L, 1L, new BigDecimal("150.00"), "Transport", LocalDate.now(), "Taxi"));
    repository.addExpense(
        new ExpenseDTO(3L, 2L, new BigDecimal("200.00"), "Shopping", LocalDate.now(), "Clothes"));
  ...
  void testGetAllExpenses() throws Exception {
    mockMvc
        .perform(get("/api/v1/expenses").param("userId", "1"))
        .andDo(print())
  ...
  void testGetExpenseById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/expenses/10")) // non-existing ID
        .andDo(print())
  ...
  void testCreateExpense() throws Exception {
    ...
    ExpenseDTO newExpense =
        new ExpenseDTO(
            null, 1L, new BigDecimal("75.00"), "Groceries", LocalDate.now(), "Weekly shopping");
  ...
  void testUpdateExpense() throws Exception {
    ExpenseDTO updatedExpense =
        new ExpenseDTO(1L, 1L, new BigDecimal("55.00"), "Food", LocalDate.now(), "Lunch updated");
  ...
  void testDeleteExpense() throws Exception {
    mockMvc.perform(delete("/api/v1/expenses/2")).andDo(print()).andExpect(status().isOk());
